...
//    private val setImagesUseCase: BaseUseCase<List<ImageSetParam>, Unit>,
    ...
    override fun onLogout() {
        setTokenUseCase
            .setResultNotifier {
                uiState.state = when (it) {
                    is Rezult.Failure -> HomeUiState.State.OnNotification(it.err.status.code.toString())
                    is Rezult.Success -> HomeUiState.State.LoggedOut()
                }
            }
            .setLifeCycleNotifier {
                when (it) {
                    BaseUseCase.LifeCycle.OnStart -> HomeUiState.State.Loading
                    BaseUseCase.LifeCycle.OnFinish -> HomeUiState.State.Idle
                }
            }
